...
type AppStack t = SyncT t (TimerT t (NetworkT t (LoggingT t (GameMonad t))))
...
deriving instance (ReflexHost t, MonadCatch (HostFrame t), MonadBaseControl IO (HostFrame t), MonadIO (HostFrame t)) => NetworkServer t (AppMonad t)
